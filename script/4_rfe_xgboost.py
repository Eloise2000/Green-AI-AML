...
feature_importance = ["Is_laundering", "Sender_account", "Receiver_account", "Amount", "Payment_currency",
                    "Received_currency", "Receiver_bank_location", "Payment_type", 
                    "Date_Day", "Hour", "Date_Month", "Date_Year", "Sender_bank_location"]

# Check if the number of features is provided
if len(sys.argv) != 2:
    raise ValueError("Number of features to select must be specified as an argument.")

# Get the number of features to use from the command line argument
num_features = int(sys.argv[1])

# Check if the number of features is within the valid range
if num_features <= 0 or num_features > len(feature_importance):
    raise ValueError(f"Number of features must be between 1 and {len(feature_importance)}.")

# Select the corresponding features from the list based on the number
selected_features = feature_importance[:num_features+1] # Add label in
